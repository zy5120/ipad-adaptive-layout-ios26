...
适配时**原则上尽量用 `.fullScreen`**：主内容 / 详情 / 从列表进入的内容一律 `.fullScreen`（竖屏 push、横屏副屏）。只有**特定的简单弹层**（轻量选择器、快捷输入、临时小弹窗）才保留 `.sheet`。具体取舍交给实现者（AI）按页面内容判断：能进副屏就 `.fullScreen`，只有“随手开、随手关”的轻量 UI 才用 `.sheet`。
