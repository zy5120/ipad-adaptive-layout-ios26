...
### 坑 4：push 页面控件点击被“焦点跟随”TapGesture 吞掉（iOS 26/27）

**现象**：案件/委托人/参与人等 push 进导航栈的表单页里，选择器、日期、电话、公司搜索按钮**点击有反馈但动作不执行**；文本框正常；根页面（列表/详情/更多首页）正常。构建 530 全部正常，588 加入焦点跟随手势后全部点不动。

**根因**：Tab 根视图 / 副屏挂 `TapGesture().onEnded { pad.focusedPane = ... }` 做“点击任意处切换焦点”，触摸期间改全局状态触发整树重绘，iOS 26/27 会取消 push 页里原生控件的点击动作（与代码注释记载的“按下即触发状态变更导致行按钮点击被取消”同一机制；DragGesture 用 `minimumDistance: 4` 规避过，TapGesture 恰好绕过该规避）。

**铁律（鱼律 2026-08-07 实测）**：
- 焦点跟随**只用 `DragGesture(minimumDistance: 4)`**（值变化才赋值），**禁止用 TapGesture 切换 focusedPane**；直接移除最可靠，`DispatchQueue.main.async` 延后仍不稳。
- 不要用“把 Picker 改成 Menu”绕过——只修好选择器，日期/电话/公司搜索仍坏，且偏离原生行为；根因移除后原生 Picker 直接可用。
- 判据：根页面正常、push 页面整页控件点不动、文本框正常 = 根视图 TapGesture 焦点手势吞点击。
...
- **草稿必须包含全部状态**：不止文本字段，还包括分段选择器/类型切换（如参与人的「自然人/法人」）——旋转后类型被重置会导致该类型下的子功能（如法人专属的搜索公司）一起消失。
- **子页被覆盖也会触发父页 onDisappear**：父表单内推入/弹层打开子页（如搜索公司）时，父表单的 onDisappear 会触发——此时**不能**清会话；会话清理只放在「编辑上下文关闭」（带旋转标记区分旋转 vs 用户关闭），否则会误清导致页面循环出现/消失。
...
### 表单页禁止自带 NavigationStack（旋转残留/重复/背景异常的根源）

**现象**：编辑案件页（民事/刑事）在横竖屏切换时，参与人表单/搜索公司跑到左屏、出现重复实例、页面底部背景异常；新建案件页完全正常。

**根因**：编辑案件页自带一层 `NavigationStack`（历史遗留：以前可能以弹层形式出现），子页（参与人/搜索公司）的推入发生在这层**内嵌栈**里；旋转时外层栈 pop，但内嵌栈里的推入残留在旧容器，新容器再推一份 → 重复/跑错容器。

**铁律**：表单/详情页**不要自带 NavigationStack**——竖屏 push、横屏副屏都由外层容器（Tab 栈 / SplitDetailPane / 更多钻取栈）统一管理，页面 body 直接返回 Form/List 内容即可。如果页面曾有「弹层呈现」需求（需要 X/取消按钮），用 `presentedAsSheet` 参数控制 toolbar，而不是包导航栈。

排查方法：对比「正常页面 vs 出问题页面」的 body 结构，先看是否多了 `NavigationStack` 包裹。
